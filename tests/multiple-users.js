...
(async () => {
  ...
  await p1.type('#player_name', 'Player1');
  await p1.click('#create_town');

  // Rest players joins to the town
  await p2.goto(`http://localhost:5000/towns/Swiebodzin`);
  await p2.type('#player_name', 'Player2');
  await p2.waitForSelector('#join_town')
  await p2.click('#join_town');

  await p3.goto(`http://localhost:5000/towns/Swiebodzin`);
  await p3.type('#player_name', 'Player3');
  await p3.waitForSelector('#join_town')
  await p3.click('#join_town');

  await p4.goto(`http://localhost:5000/towns/Swiebodzin`);
  await p4.type('#player_name', 'Player4');
  await p4.waitForSelector('#join_town')
  await p4.click('#join_town');

  await p5.goto(`http://localhost:5000/towns/Swiebodzin`);
  await p5.type('#player_name', 'Player5');
  await p5.waitForSelector('#join_town')
  await p5.click('#join_town');
